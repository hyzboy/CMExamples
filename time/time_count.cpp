...
int main(int,char **)
{
    ...
    Date dy1(2021,1,1); cout << "2021-01-01 -> " << dy1.DayOfYear() << " (expect 1)" << endl;
    Date dy2(2021,2,28); cout << "2021-02-28 -> " << dy2.DayOfYear() << " (expect 59)" << endl;
    Date dy3(2020,2,29); cout << "2020-02-29 -> " << dy3.DayOfYear() << " (expect 60, leap year)" << endl;
    Date dy4(2019,12,31); cout << "2019-12-31 -> " << dy4.DayOfYear() << " (expect 365)" << endl;
    PrintSeparator();

    // DayOfWeek tests
    cout << "DayOfWeek tests (GetWeekDay):" << endl;
    Date wd1(2021,1,1); cout << "2021-01-01 -> " << WeekDayName(wd1.GetWeekDay()) << " (expect Friday)" << endl;
    Date wd2(2021,9,23); cout << "2021-09-23 -> " << WeekDayName(wd2.GetWeekDay()) << " (expect Thursday)" << endl;
    Date wd3(2000,1,1); cout << "2000-01-01 -> " << WeekDayName(wd3.GetWeekDay()) << " (expect Saturday)" << endl;
    Date wd4(1970,1,1); cout << "1970-01-01 -> " << WeekDayName(wd4.GetWeekDay()) << " (expect Thursday)" << endl;
    Date wd5(2020,2,29); cout << "2020-02-29 -> " << WeekDayName(wd5.GetWeekDay()) << " (expect Saturday)" << endl;
    PrintSeparator();

    // SetMonth / SetDay edge cases
    cout << "SetMonth / SetDay edge cases:" << endl;
    Date dm1(2021,1,31); cout << "start: 2021-01-31" << endl;
    ...
    Date ly1(2000,2,28); ly1.SetDay(29); cout << "2000-02-28 SetDay(29)-> "<<ly1.GetYear()<<'-'<<ly1.GetMonth()<<'-'<<ly1.GetDay()<<" (2000 is leap)"<<endl;
    Date ly2(1900,2,28); ly2.SetDay(29); cout << "1900-02-28 SetDay(29)-> "<<ly2.GetYear()<<'-'<<ly2.GetMonth()<<'-'<<ly2.GetDay()<<" (1900 not leap)"<<endl;
    PrintSeparator();

    // Time microsecond and overflow tests
    cout << "Time microsecond / overflow tests:" << endl;
    Time tm1; tm1.Set(1,2,3,500000);
    cout << "tm1 initial: 01:02:03." << tm1.GetMicroSecond() << endl;

    Time tm2; tm2.Set(23,59,59,999999);
    cout << "tm2 initial: 23:59:59."<<tm2.GetMicroSecond()<<" (max before roll)"<<endl;

    // Large microsecond (overflow into seconds)
    Time tm3; tm3.Set(0,0,0,0);
    ...
    Date ed(1999,12,31); Time et(23,59,59,123456);
